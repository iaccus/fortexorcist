...
function FW:GetAllTimersFrames()
	local t = {}
	for i=1,instances.rows,1 do		
		for n=1, #instances[i][2].groups,1 do
			for k=1, #instances[i][2].groups[n].bars, 1 do				
				t[#t+1] = instances[i][2].groups[n].bars[k]
			end
		end
	end
	return t
...
function ST:AddSpell(spell, duration, group, flags)
	local s,t,id = FW:SpellName(spell);
	current_spell = s;
	flags = flags or 0;
...
function ST:CheckSpellID(spellid)
	checkSpellID[current_spell] = spellid;
	return self;
end
...
function ST:RemoveTimer(spell, guid)
	local i=1;
	while i<=st.rows do
		if st[i][8] == spell and st[i][11] == guid then
			
		--	print("Remove ", spell, guid)
			st:remove(i);
			break;
		else
			i = i+1
...
local function ST_ScanForMissing(unit,guid,filter)	
	...
	local i=1;
	while true do
		local spell,rank,texture,stacks,_,total,expire,caster,isStealable,shouldConsolidate, spellId  = UnitAura(unit,i,filter);
		
		if spell then
			if isMine[caster] and ( ( checkSpellID[spell] and checkSpellID[spell] == spellId ) or ( not checkSpellID[spell] and true ) ) and (Track[spell] and Track[spell][7] or 0) == (filter == "HELPFUL" and 1 or 0) then
...
local function UnitHasYourAura(unit,buff,filter)
	...
	--name 1, rank 2, icon 3, count4 , debuffType 5, duration 6, expirationTime 7, unitCaster 8, isStealable 9, shouldConsolidate 10, spellId 11
	local b = 1;
	local _,d,t,m,s,tt,sID;--buff name, texture
	while true do
		d,_,_,s,_,tt,t,m,_,_,sID = UnitAura(unit,b,filter);
		if d then
			if d == buff and isMine[m] and ( ( checkSpellID[d] and checkSpellID[d] == sID ) or ( not checkSpellID[d] and true ) ) then
				return t,b,s,tt;
			end
			b=b+1;
		else
...
local function ST_ScanWeaponEnchant()
	local hasMainHandEnchant, mainHandExpiration, mainHandCharges, mainHandNumberValue, 
			hasOffHandEnchant, offHandExpiration, offHandCharges, offHandNumberValue, 
	...
	frame.Draw = function(self)
		...
		for k=1,links.rows do -- stuff to make it 'skip' certain timers for display
			...
			if vis > 0 then -- is visible
				...
				-- ALL OTHER STUFF
				if t19 == 0 or not s.RaidTargets[0] or not FW_RaidIconCoords[t19] then
					bar.raidicon:Hide();
					group.raidicon:Hide();
				else
...
do
	...
	local function ST_CombatLogEvent(event,...)
		...
		if select(5,...) == PLAYER then
			...
			elseif FW.Settings.CLEU and arg2 == "SPELL_AURA_APPLIED" or arg2 == "SPELL_AURA_REFRESH" then
				
			--	print("CLUE ON OFF ", FW.Settings.CLEU)
				
				if Track[select(13,...)] and Track[select(13,...)][1] == 0 and Tick[select(13,...)] then
				--	print("Add SPELL ", select(13,...)..select(9,...)..select(8,...))
					
					ST:AddNewTimer(
					GetTime()+Track[select(13,...)][2],
					Track[select(13,...)][2],
					select(9,...),
					select(13,...),
					0,
			...
			elseif FW.Settings.CLEU and arg2 == "SPELL_AURA_REMOVED" then
				if Track[select(13,...)] and Tick[select(13,...)] then
					ST:RemoveTimer(select(13,...), select(8,...))
				end
...
:SetMainCategory(FWL.ADVANCED,FW.ICON.DEFAULT,99,"DEFAULT")
	:SetSubCategory(FWL.SPELL_TIMER,FW.ICON.DEFAULT,3)
		...
		:AddOption("CHK","Combat log",		"Turn on/off COMBAT_LOG_EVENT_UNFILTERED parsing for dots","CLEU")
