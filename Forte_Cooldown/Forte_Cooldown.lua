...
local function MyGetSpellBaseCooldonw(spellID)
	
	local basecd = GetSpellBaseCooldown(spellID)
	
	if basecd and basecd == 0 and mycooldowns[spellID] then 
...
local cooldowns;
function CD:CheckCooldown(spell,start,duration,texture,flag, stacks)
	--if not FW.Settings then return; end
	if stacks then
		spell = spell.."x"..stacks
	end
	...
	--end
	--[[
	if spell == 'Берсерк' then
		print('T1', spell, start, duration)
		print('T2', GetSpellCooldown(spell))
		print('T3', texture)
	end
	]]
	local end_time = start+duration;
	if cooldowns[spell] and cooldowns[spell][3] == texture then
		if start ~= cooldowns[spell][1] or duration ~= cooldowns[spell][2] then -- something changed in the duration
		--[[	assert(start <=15, "Error")
			assert(cooldowns[spell][1] <= 15, "Error")
			
			print(start, cooldowns[spell][1])
			print(duration, cooldowns[spell][2])]]
			if duration == 0 then -- no event fired when cd is over :|
				index = cd:find3(spell,1, flag,6, 0,15) or cd:find3(spell,1, flag,6, 1,15);
				
			--	spellname_cd_index[spell]
				
		--		print(start, duration, spell, start, index, spellname_cd_index[spell])
				if index then
					if cd[index] and cd[index][1] == spell then  
...
local function CD_CreateCooldowns(frame)
	...
	while i<=cd.rows do
		...
		if t17 == 0 then -- determine if this timer will be visible
			...
			--		print("keeping focus on "..i.." "..cd[i][1])
					focus[group] = i;
				else
					cd[i][14] = 0;
					cd[i][15] = 0;
	...
	for g,n in ipairs(first) do
		if not focus[g] then
	--		print("correcting focus"..n.." "..cd[n][1])
...
local function CD_ScanActionCooldowns()
	...
	--	print(what, id)
		if what == "item" then
		--	if BanItems[id] then return end
			spell,_,_,_,_,_,_,_,_,texture = GetItemInfo(id);
			if spell then
				start,duration,applies = GetItemCooldown(id);
				if applies == 1 then
		...
		--	end
			...
			local currentCharges, maxCharges, cooldownStart, cooldownDuration = GetSpellCharges(id)
				if ( maxCharges and ( cooldownStart > 0 ) and ( cooldownDuration > 1.5 ) ) then
					if ( maxCharges ~= currentCharges) then
				--		print("CD_ScanActionCooldowns spell YES max charge", cooldownStart, cooldownDuration, spell, enabled)
						CD:CheckCooldown(spell,cooldownStart,cooldownDuration*(maxCharges-currentCharges),texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL, currentCharges);
					end
				elseif ( start > 0 and duration > 1.5 ) then -- Number - 0 if the spell is active (Stealth, Shadowmeld, Presence of Mind, etc) and the cooldown will begin as soon as the spell is used/cancelled; 1 otherwise. 
				--	print("CD_ScanActionCooldowns spell No max charge", start, duration, spell, enabled)
					CD:CheckCooldown(spell,start,duration,texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL);
				end
		elseif what == "macro" and false then
			local name, rank, id = GetMacroSpell(id);
			if id then
				spell = FW:SpellName(id);
				texture  = GetSpellTexture(id);
				start,duration,enabled = GetSpellCooldown(id);
			...
			--[[
				if maxCharges then
				print(duration, enabled, currentCharges, maxCharges)
			end
			]]
				if ( maxCharges and ( cooldownStart > 0 ) and ( cooldownDuration > 1.5 ) ) then
					if ( maxCharges ~= currentCharges) then
				--		print("YES max charge", cooldownStart, cooldownDuration, spell, enabled)
						CD:CheckCooldown(spell,cooldownStart,cooldownDuration*(maxCharges-currentCharges),texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL, currentCharges);
					end
				elseif ( start > 0 and duration > 1.5 ) then -- Number - 0 if the spell is active (Stealth, Shadowmeld, Presence of Mind, etc) and the cooldown will begin as soon as the spell is used/cancelled; 1 otherwise. 
				--	print("No max charge", start, duration, spell, enabled)
					CD:CheckCooldown(spell,start,duration,texture,(CD_IsSpecialCooldown(spell)) or FLAG_SPELL);
				end
			end
...
local function CD_ScanBookCooldowns()
	...
	for i = 1, offset + numSpells, 1 do
		spellName = GetSpellBookItemName(i, 'player')
		if not spellName then break end
		local slotType, spellId = GetSpellBookItemInfo(i, 'player')

		if spellId and not disable_spells[spellId] then
			if slotType == "FLYOUT" then
				...
				for fi = 1, ((isKnown and numSlots) or 0), 1 do
					...
					last = flySpellName
					if flySpellId then
						local flycd = MyGetSpellBaseCooldonw(flySpellId)
						if flycd and flycd > 0 then
							start,duration,enabled = GetSpellCooldown(i, "player");
							if start and duration then
								texture  = GetSpellTexture(flySpellId);
								local currentCharges, maxCharges, cooldownStart, cooldownDuration = GetSpellCharges(flySpellId)
								if ( maxCharges and ( cooldownStart > 0 ) and ( cooldownDuration > 1.5 ) ) then
									if ( maxCharges ~= currentCharges) then
										CD:CheckCooldown(spellName,cooldownStart,cooldownDuration*(maxCharges-currentCharges),texture,(CD_IsSpecialCooldown(spellName)) or FLAG_SPELL, currentCharges);
									end
								elseif ( start > 0 and duration > 1.5 ) then
									CD:CheckCooldown(spellName,start,duration,texture,(CD_IsSpecialCooldown(spellName)) or FLAG_SPELL);
								else
									CD:CheckCooldown(spellName,start,duration,texture,(CD_IsSpecialCooldown(spellName)) or FLAG_SPELL);
								end
							end
			...
			elseif slotType ~= "FUTURESPELL" and spellId ~= last then
				last = spellId
				local spellcd = MyGetSpellBaseCooldonw(spellId)
...
local function CD_ScanWeaponEnchant()
	local hasMainHandEnchant, mainHandExpiration, mainHandCharges, mainHandNumberValue, 
			hasOffHandEnchant, offHandExpiration, offHandCharges, offHandNumberValue, hasThrownEnchant, thrownExpiration, thrownCharges = GetWeaponEnchantInfo();
	--FW:Show("scan");
...
FW:RegisterVariablesEvent(function()
	...
	FW:RegisterToEvent("BAG_UPDATE_COOLDOWN",		function() FW:RegisterThrottle(CD_ScanBagCooldowns);end);
...
function CD:AddMeleePowerupCooldowns()
	...
	:AddHiddenCooldown(nil,96230,60) -- synapse spring
...
function CD:AddCasterPowerupCooldowns()
	...
	:AddHiddenCooldown(nil,74241,45) -- Enchant Weapon - Power Torren
end
